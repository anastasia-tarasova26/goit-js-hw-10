...
const options = {
  ...
  onClose(selectedDates) {
    ...
    if (currentTime.getTime() < selectedDates.getTime()) {
      // enables start button if chosen date is in the future
      startButton.disabled = false;
    } else {
      // disables start button if chosen date is in the past
      startButton.disabled = true;
      izitoast.show({
        // shhows alert window with message to choose the correct date
...
function timerCounter() {
  const timer = setInterval(() => {
    // interval to repeat code every second
    let currentTime = new Date();
